    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            >
              <Link href="/" className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white">Back to Blog</span>
              </Link>
            </Button>
      ...
      <article className="pt-24 pb-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          <header className="mb-12 space-y-8">
            ...
            <div className="flex flex-wrap items-center gap-3">
              {post.category && post.category !== 'General' && (
                <Badge className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white border-0 px-4 py-1.5 text-xs font-semibold tracking-wide uppercase hover:from-blue-700 hover:to-indigo-700 shadow-sm shadow-blue-500/30">
                  {post.category}
                </Badge>
              )}
            ...
            <div className="flex flex-wrap items-center gap-4 pt-6 border-t border-slate-200 dark:border-slate-800">
              <Link href={`/user/${post.author?._id}`} className="flex items-center gap-3 group">
                <Avatar className="h-12 w-12 ring-2 ring-slate-200 dark:ring-slate-700 ring-offset-2 ring-offset-white dark:ring-offset-slate-900 transition-transform group-hover:scale-105">
                  <AvatarImage
              ...
              <div className="flex flex-wrap gap-2 pt-4">
                {post.tags.map((tag, index) => (
                  <Badge
                    key={index}
                    variant="secondary"
                    className="px-3 py-1 text-xs font-medium bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 transition-colors"
                  >
                    #{tag}
                  </Badge>
                ))}
          ...
          {/* Featured Image */}
          {post.featuredImage && (
            <div className="relative mb-12 rounded-2xl overflow-hidden shadow-sm shadow-slate-900/10 dark:shadow-black/30 group">
              <div className="aspect-[21/9] sm:aspect-[16/9] lg:aspect-[21/9]">
                <img
          ...
          <div className="prose prose-lg prose-slate dark:prose-invert max-w-none
            ...
            prose-code:text-blue-600 dark:prose-code:text-blue-400 prose-code:bg-slate-100 dark:prose-code:bg-slate-800 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:font-medium prose-code:before:content-none prose-code:after:content-none
            prose-pre:bg-slate-900 dark:prose-pre:bg-slate-950 prose-pre:border prose-pre:border-slate-800 prose-pre:shadow-sm
            prose-blockquote:border-l-4 prose-blockquote:border-blue-500 prose-blockquote:bg-slate-50 dark:prose-blockquote:bg-slate-900 prose-blockquote:py-2 prose-blockquote:px-6 prose-blockquote:rounded-r-lg prose-blockquote:not-italic prose-blockquote:font-normal
            ...
            <Card className="bg-gradient-to-br from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
              <CardContent className="p-8">
                <div className="flex flex-col sm:flex-row items-start gap-6">
                  <Avatar className="h-20 w-20 ring-4 ring-white dark:ring-slate-800 shadow-sm shrink-0">
                    <AvatarImage
                      src={post.author?.avatar}
                      alt={post.author?.username}
                    />
          ...
          <div className="mt-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <Link href="/" className="group block">
                <Card className="overflow-hidden border-slate-200 dark:border-slate-800 hover:shadow-sm transition-all duration-300 hover:-translate-y-1">
                  <CardContent className="p-0">
